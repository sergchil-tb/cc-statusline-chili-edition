...
mini_bar() {
    ...
    for ((i=filled; i<width; i++)); do out+="\033[38;5;244m▱"; done
    printf '%b\033[0m' "$out"
}
pct_clr() {
    local p="${1:-0}"
...
else THINK_TOK="\033[38;5;248m✦\033[0m"; fi
case "$EFFORT" in
  ...
  low)        EFFORT_TOK="\033[38;5;248m$EFFORT\033[0m" ;;
  *)          EFFORT_TOK="\033[38;5;248m$EFFORT\033[0m" ;;
esac
...
if [ -n "$USAGE" ]; then
    ...
    if [ -n "$f_util" ]; then
        ...
        seg="⏱️ $(mini_bar "$p" 14) $(pct_clr "$p")${p}%\033[0m"; [ -n "$r" ] && seg+="\033[38;5;248m ⟳ ${r}\033[0m"
        add_rate "$seg"
    fi
    if [ -n "$w_util" ]; then
        p=$(printf "%.0f" "$w_util" 2>/dev/null || echo 0); r=$(fmt_reset "$(echo "$USAGE" | jq -r '.seven_day.resets_at // empty')")
        seg="📅 $(mini_bar "$p" 14) $(pct_clr "$p")${p}%\033[0m"; [ -n "$r" ] && seg+="\033[38;5;248m ⟳ ${r}\033[0m"
        add_rate "$seg"
    fi
    ...
    if [ "$extra_on" = "true" ]; then
        ...
        CREDIT_LINE="💳 $(mini_bar "$epi" 14) $(pct_clr "$epi")\$${eud}\033[38;5;248m/\033[0m\$${eld} ${epi}%\033[0m"
    fi
fi
...
printf '\033[38;5;111;1m%s\033[0m\033[38;5;248m%s\033[0m\033[38;5;248m │ \033[0m\033[38;5;111m📁 %s\033[0m' "$MODEL_BASE" "$MODEL_TAG" "$DIR"
[ -n "$BRANCH" ] && printf '\033[38;5;248m │ \033[0m\033[38;5;176m🌿 %s\033[0m' "$BRANCH"
printf '\n'
